...
# Get historical data for MATIC up to the current date
matic_data = cg.get_coin_market_chart_range_by_id(id='matic-network',
                                                vs_currency='usd',
                                                from_timestamp=datetime.strptime(three_months_ago, '%d-%m-%Y').timestamp(),
                                                to_timestamp=datetime.now().timestamp())

# Extracting price data
prices = matic_data['prices']
...
plt.title('Matic Price Sentiment Distribution Over the Last 3 Months')
